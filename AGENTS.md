...
- Use `0.x` versions for text-token DSL experiments: human label `0.85`, filesystem label `0_85`.
- Use `1.x` versions for relational-object experiments: human label `1.103`, filesystem label `1_103`.
- Treat `0.85` as the mature text-token compact-action baseline with TASK headers.
- Treat `1.102` as the current best relational-object OOD checkpoint target in the TRM repo.
- Treat `1.103` as the first all-family 15-17 extra-hard relational stress export/eval.
- Never rename old `v1` through `v22`, `0.x`, or `1.x` configs, data, reports, or logs. They are historical references.
...
## Relational Representation Direction

The relational-object line is now the main General Pure Reasoner direction.
Text DSL remains useful for inspection and source-of-truth JSONL, but model-facing
relational arrays should remove arbitrary object names from learned input.

Core principle:

```text
learned vocabulary = roles, relations, operations, state predicates, error codes
anonymous objects  = entities, relation symbols, rules, propositions, cells,
                     values, functions, term nodes, hypotheses
```

For relational work:

- Keep entities, relation symbols, rules, propositions, cells, values, functions,
  and term nodes as anonymous object rows.
- External names such as `e17`, `r4`, `p2`, `f0`, or `h8` belong in JSONL text
  and renderer/debug metadata, not as learned model tokens.
- Alpha-renaming arbitrary symbols should not change the relational `model_view`.
- Prefer generic task/schema predicates over family-specific predicates.
- Use task-declared schema facts to describe query roles, state roles, source
  facts, action effects, done conditions, verifier meaning, and local support.
- If a new family can be described with existing generic predicates and ops, do
  not add new model-facing predicate or operation IDs.
- If a new predicate/op is genuinely needed, make it reusable, classify it in
  `relational_task_schema_arrays.py`, and add tests explaining why the existing
  schema was insufficient.

This is the structured version of the `0.85` TASK-header lesson. The model
should learn reusable schema language and anonymous object selection, not
memorize object names or opaque family IDs.

Current relational keeper path:

- Source JSONL is still generated by normal DSL configs.
- Model-facing arrays are exported with:

```bash
python3 -m reasoning_dsl.cli relational-task-schema-arrays \
  --data-dir data/<jsonl-source-dataset> \
  --output-dir data/symbolic-1_XX-<name>-arrays \
  --fail-on-legacy-predicates
```

- Keeper relational arrays should have `active_legacy_fact_predicates: []`.
- Keep `fact_pred_ids`, `op_ids`, `mode_ids`, `validity_ids`, and `error_ids`
  compatible with the checkpoint being evaluated. For eval-only hard holdouts,
  include the baseline train/test JSONL during export if needed so rare verifier
  error IDs stay aligned.

Known current relational datasets:

- `symbolic-1_91-generic-state-facts-arrays`: current five-family generic schema
  train/test/OOD benchmark.
- `symbolic-1_103-extra-hard-15_17-relational-compatible-arrays`: all-family
  15-17 extra-hard relational stress export.

Known current relational checkpoint targets in the TRM repo:

- Best balanced direct: `relational-1_95-generic-state-facts-term-action-ft`.
- Best OOD/direct parity: `relational-1_102-msg4-graph-verify-ft`.
...
- Text TRM exports are for text-token experiments. Relational-object experiments
  should use `relational-task-schema-arrays`.
- Relational exports must preserve anonymous-object invariance: arbitrary symbol
  renaming changes external names only, not object/fact/target arrays.
- For keeper relational exports, run with `--fail-on-legacy-predicates` unless
  the user explicitly asks for a legacy/prototype comparison.
...
3. Add or extend a text-source config named like `configs/symbolic_0_XX_<short_name>.json`.
4. Add focused generator tests covering:
   ...
   - symbol offsets
5. Add relational tests when the family is intended for General Pure Reasoner work:
   - `task_schema_record` produces anonymous objects, not learned symbol names.
   - `model_view(task_schema_record(example))` is invariant under alpha-renaming.
   - source facts, state facts, action targets, repair targets, and verifier
     labels are represented with generic schema roles where possible.
   - `records_to_task_schema_arrays` exports cleanly with no active legacy
     predicates for keeper datasets.
6. Add export or audit tests if the family changes dataset shape, vocab behavior, relational metadata, or verifier error IDs.
7. Run tests before generation:
...
8. Generate and audit the text dataset:
...
9. Export relational arrays when the experiment is part of the relational GPR path:

```bash
python3 -m reasoning_dsl.cli relational-task-schema-arrays \
  --data-dir data/symbolic-0_XX-<short-name> \
  --output-dir data/symbolic-1_XX-<short-name>-arrays \
  --fail-on-legacy-predicates
```

## Current Baseline

- Stable text-token direction: `0.85` compact actions with TASK headers.
- Stable relational-object direction: `1.91+` generic task-schema/state-fact arrays.
- Current relational OOD/direct target: `1.102` in the TRM repo.
- Current relational hard-holdout export: `1.103` all-family 15-17.
- Stable families: graph reachability, implication chains, relation composition,
  tree ancestry, and term rewriting.
- Current modes: `action`, `repair_action`, `verify`.
- New families should be designed for relational-object export from the start,
  even if text JSONL remains the human-readable source format.
